...
frappe.listview_settings["Pennylane Customer Invoice"] = {
	...
	onload(listview) {
		listview.page.add_inner_button(__("Sync All"), () => {
			pennylane_sync_all(listview, "Pennylane Customer Invoice");
		});

		frappe.call({
			method: "frappe.client.get_count",
			args: { doctype: "Pennylane Customer Invoice", filters: { sync_status: "Deleted" } },
			callback(r) {
				if (r.message > 0) {
					const btn = listview.page.add_inner_button(__("Clean up Deleted"), () => {
						pennylane_cleanup_deleted(listview, "Pennylane Customer Invoice", btn);
					});
				}
			},
		});
